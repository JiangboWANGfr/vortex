...
//
// Host driver for the AES-256-GCM throughput / overhead benchmark. Generates
// num_tasks buffers of bytes_per_task each under a shared key, runs the kernel,
// and folds the per-task tags into a single checksum. SOFTWARE and NATIVE must
// produce the same checksum (cross-check); UNPROTECTED folds the moved data.
// The framework's "PERF: ... cycles=N" line carries the timing; this driver
// adds "GCM_BENCH: ... checksum=0x...".
...
void show_usage() {
  std::cout << "Vortex AES-256-GCM throughput benchmark." << std::endl;
...
int main(int argc, char** argv) {
  ...
  std::cout << "GCM_BENCH: num_tasks=" << num_tasks
  ...
  RT_CHECK(vx_mem_alloc(device, sizeof(gcm_bench_status_t), VX_MEM_READ_WRITE, &status_buffer));
  RT_CHECK(vx_mem_address(status_buffer, &kernel_arg.status_addr));
  kernel_arg.num_tasks = num_tasks;
  kernel_arg.bytes_per_task = bytes_per_task;

  gcm_bench_status_t status = {};
  ...
  std::cout << "GCM_BENCH: checksum=0x";
  for (int i = 0; i < 16; ++i)
    std::cout << std::hex << std::setw(2) << std::setfill('0') << (int)checksum[i];
  std::cout << std::dec << std::endl;

  if (status.errors != 0) {
    std::cout << "GCM bench FAILED: errors=" << status.errors << std::endl;
    return (int)status.errors;
  }
  std::cout << "GCM bench PASSED (" << status.completed_tasks << " streams, "
