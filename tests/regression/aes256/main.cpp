...
static void show_usage() {
  std::cout << "Vortex AES256 regression." << std::endl;
...
int main(int argc, char** argv) {
  ...
  RT_CHECK(vx_mem_alloc(device, sizeof(aes256_status_t), VX_MEM_READ_WRITE, &status_buffer));
  ...
  if (status.errors != 0) {
    std::cout << "AES256 FAILED: errors=" << std::dec << status.errors
              << ", failed_mask=0x" << std::hex << status.failed_mask << std::endl;
    return status.errors;
  }

  std::cout << "AES256 PASSED!" << std::endl;
  return 0;
}
