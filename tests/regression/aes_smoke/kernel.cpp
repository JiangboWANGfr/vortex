...
int main() {
  ...
  aes_smoke_status_t* status = (aes_smoke_status_t*)arg->status_addr;

  int scalar_errors = check_scalar_ops();
  int round_errors = check_round_helpers();

  status->errors = scalar_errors + round_errors;
  status->failed_mask = 0;
  if (scalar_errors)
    status->failed_mask |= AES_SMOKE_FAIL_SCALAR;
  if (round_errors)
    status->failed_mask |= AES_SMOKE_FAIL_ROUND;
